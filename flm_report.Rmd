...
```
This section summarizes the results for the chosen AOI, located in the municipality(ies) of **`r unique(adm_bound$NAME_2)`**, in the state(s)/province(s) of **`r unique(adm_bound$NAME_1)`**, in the country(ies) of **`r unique(adm_bound$COUNTRY[1])`**, respectively. 
...
forest_type_perc<- area_pixels_forest_type |> mutate(total_area = sum(area), perc = round(100*area/total_area,2))
```
...
ggcolors<- my_coltab |> dplyr::mutate(hex = grDevices::rgb(red, green, blue, maxColorValue = 255)) |> 
    inner_join(levels(r_masked)[[1]], by = join_by(value == ID))
...
my_color_scale<- scale_colour_manual(name = "value",values = my_colors)
my_fill_scale<- scale_fill_manual(name = "value",values = my_colors)

my_barplot<- function(data, ...){
    ggplot() + 
    geom_bar(aes(x = value, y = area, fill = value), 
             data = data,
             stat = "identity") + 
    theme_minimal() + 
    my_fill_scale + 
    labs(x = "", y = "area (ha)", fill = "") + 
...
The top 5 `patch` metrics where selected for creating maps. The `patch` metrics calculated were: **`r plot_files |> fs::path_file() |> fs::path_ext_remove() |> gsub("_", " ", x = _) |> paste0(collapse = ", ")`**. `Id` is the patch id, and `category` is the land cover. For more information on the other metrics, go [here](https://r-spatialecology.github.io/landscapemetrics/reference/index.html#patch-level-metrics).

The category map show the land cover classification used in the FLM metrics calculation. This is the land cover map from the Copernicus Global Land Servive ([link](https://land.copernicus.eu/global/products/lc)). For calculating the FLM metrics, only the forest classes are used, and all other layers are treated as a single non-forest category.
